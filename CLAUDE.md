...
tests/                # pytest suite (1,119: 1,042 fast + 77 browser). Markers in
...
.venv/bin/python -m pytest    # 1,119; data-dependent ones skip if artifacts missing
...
- **One relation mechanism: `MM.relate(row, rel)`.** The controls render inside `buildCardDetailHtml`, so every panel that shows a card has them; the dispatcher branches the graph in Discover/Walk and opens a browse set in explore. The old Find Similar / Find Synergies pair is deleted — it took no card argument, read `selectedCards`, and was therefore a silent no-op in three of five panels, wrong-carded in a fourth, and threw under `?renderer=canvas`. If you add a panel that renders card detail, pass the **row**, not just the record.
