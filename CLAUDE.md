...
- **THE FORGE AI WILL NOT PRESS A SACRIFICE BUTTON, so every Forge result on a sacrifice deck is a FLOOR.** Measured on one 400-game run: Ashnod's Altar cast 46 times and activated **0**; every sac outlet sits at 0.00-0.07 activations per casting, free ones included. Skullclamp at **1.25** is the control -- the AI uses equipment happily. A card whose value is behind an ACTIVATED ability is under-measured; check `activated <card>` against `cast <card>` before reading a result that rests on one, and prefer a TRIGGER to an activation for the same effect. -> `docs/gotchas-bench.md`
