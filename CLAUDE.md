...
tests/                # pytest suite (1,130: 1,039 fast + 91 browser). Markers in
...
.venv/bin/python -m pytest    # 1,130; data-dependent ones skip if artifacts missing
...
- **`session.js` is the one answer to "what am I holding".** There were EIGHT set-of-cards containers and SEVEN answers to "which card is selected" (`selectedCards[topCardIndex]`, `browseSet.indices[pos]`, `orientation.anchor`, `Discovery.current`, `Force.pinned`, `hovered`, `deckState.commander`) with nothing reconciling them — that is *why* a click meant something different in every mode. Session owns **focus** and the **tray**. It deliberately does NOT own the graph's storage: `force.js` holds nodes as live d3 bodies mutated every tick, so a second membership array would recreate the duplicate-model problem — instead force **registers itself as Session's graph provider** (`Session.useGraph`) and everything else asks Session. Still out of scope on purpose: `deckState`, `DeckMap.active`, `Drill.indices` (different concepts), and `browseSet` (box-select is a different question).
- **Explore's lens is LIVE, not a snapshot.** `orientation` used to hold `{rows: Set, anchor}` copied from `Force.rows()` at mode-entry and never updated, so the atlas showed a photograph of your walk — grow the graph and nothing moved until you left and came back. It now holds only `{label}`; membership comes from `Session.rows()` on every render and the anchor from `Session.focus`. This is most of why Explore felt inert, and no styling would have fixed it.
- **`stage.js` owns the surface; it never stores a coordinate.** The two canvas renderers (`render/canvas.js` for the 34K atlas, `force.js` for the graph) shared *zero* lines while separately implementing canvas+DPR resize, d3-zoom, the draw prologue, world↔screen, fit-to-extent and greedy AABB label collision. Stage holds all of it. The reason it does not own positions: force mutates node x/y every tick, the atlas never mutates points and moves only the transform — an abstraction owning both would be a union of two designs. Label placement is **incremental** (`Stage.placer`) because The Walk draws on accept and caps on accepted, sharing one collision set between edge and node labels; `placeLabels` is a batch wrapper for the atlas.
- **`mode:'edges'` ≠ `mode:'lines'`.** A `lines` layer is one flattened polyline, one colour, excluded from the quadtree — fine for Deck Lens verified lines, useless for "this edge is a synergy, that one an obsolescence". `edges` carries `[{source:[x,y], target:[x,y], rel, reason, d}]` and inks via `Stage.INK`, so an edge means the same thing on the map as on the graph. Coordinates are explicit, not row ids — the renderer knows nothing about cards. Edge layers are excluded from the quadtree AND the legend. Test it by **reading pixels**; "the layer is present" passes while nothing draws.
- **One surface per mode, and the CSS must name the RIGHT one.** `#plot.force-mode` hid `.js-plotly-plot`/`.plot-container`/`.svg-container` — Plotly elements that stopped existing when Plotly was deleted — so in Discover and The Walk the 34K atlas kept drawing underneath with its labels and legend. It survived a mode-by-mode check because `.map-canvas` is created lazily on the first Explore render: boot straight into Discover and there is nothing to leak. **Test the ordering that exposes a leak (Explore first), not the one that hides it.**
...
- **One relation mechanism: `MM.relate(row, rel)`, and it always grows the graph.** The controls render inside `buildCardDetailHtml`, so every panel that shows a card has them — including the **Keep** button, since keeping a card you found in the atlas is the same act as keeping one you walked to. From Explore, `relate` switches to Discover seeded on that card: Explore is a **launchpad**, not a second mode with its own answer to the same button. It used to fork (browse set in explore, branch in graph modes) and that fork is deleted — one control meaning two things is why the atlas felt dead next to the walk. Box-select still opens a browse set; that is a different question. The older Find Similar / Find Synergies pair is long gone — it took no card argument, read `selectedCards`, and was a silent no-op in three of five panels, wrong-carded in a fourth, and threw under `?renderer=canvas`. If you add a panel that renders card detail, pass the **row**, not just the record.
