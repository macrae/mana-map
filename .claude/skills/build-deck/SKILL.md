...
1. **Baseline** (free, no agents): `.venv/bin/manamap pilot build-deck <slug>`.
   Writes `build_plan.json`. Then `.venv/bin/manamap pilot validate-build <slug>`.
   If this fails, stop — it is a code or brief problem, not something an agent fixes.
...
3. **Pool**: spawn `deck-analyst` with the slug, the brief, and the target bracket.
   Write its JSON to `candidate_pool.json`. This is the architect's sandbox — the
...
8. **Materialise**: `.venv/bin/manamap pilot build-deck <slug> --write-decklist`,
   then `.venv/bin/manamap pilot fetch-deck <slug>`, then
   `.venv/bin/manamap pilot validate-deck <slug>` — the real legality gate, the same
   one every hand-built deck passes.
