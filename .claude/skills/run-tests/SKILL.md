...
```bash
.venv/bin/python -m pytest                # full suite: 364 tests, discovery via tests/
...
- **103 pilot-subsystem tests** (`test_pilot_*.py`) are mostly pure-function; the data-gated ones auto-skip via `requires_rules` / `requires_deck` / `requires_strategy` markers (build the rules DB, fetch goblin-storm, or run `manamap pilot build-strategy-db` respectively to unskip). See `docs/testing.md` for the per-file table.
