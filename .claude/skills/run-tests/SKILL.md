...
```bash
.venv/bin/python -m pytest                # full suite: 686 tests, discovery via tests/
.venv/bin/python -m pytest tests/test_synergy.py -v    # single file
```

## Interpreting results

- **276 card-pipeline unit tests** need no data files and must always pass.
...
- **368 pilot-subsystem tests** (`test_pilot_*.py`) are mostly pure-function; the data-gated ones auto-skip via `requires_rules` / `requires_deck` / `requires_strategy` markers (build the rules DB, fetch goblin-storm, or run `manamap pilot build-strategy-db` respectively to unskip). See `docs/testing.md` for the per-file table.
