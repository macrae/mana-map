...
```bash
.venv/bin/python -m pytest              # everything (1,124, ~7 min)
...
1,124 tests in `tests/`: 438 card-pipeline + 601 pilot-subsystem + **85 browser**.
...
### Browser tests (85) — `tests/test_viz_behaviour.py` + `test_decklist_parity.py`
