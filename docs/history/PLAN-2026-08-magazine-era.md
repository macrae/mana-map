> **HISTORICAL (archived 2026-08-19).** This is the PLAN.md of the magazine era, kept verbatim for the record — the nine issues, the front of the book, the length budgets, the Phase 3 work. The live plan is `PLAN.md`; the vision it was superseded by is `docs/vision.md`. Figures in here were true when written and are not maintained.
...
`resolve-stack`.** Road B (own engine) shelved. Phases S1–S5 in the doc; **S1, S2 and S4 are done** (`simulate` — now SEEDED, `-s`
was in Forge's source — the `forge` marker, the parser with CIs and two honest token
