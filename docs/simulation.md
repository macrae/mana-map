...
| **(c) reproducible** | **NO at the byte level; YES statistically** | no seed flag or preference exists; two identical 2-seat runs gave different winners and turn counts. A Forge run is a **sample of N games**, never a replayable game |
...
data/decks/<slug>/sim/logs/<run-id>/*.log           gitignored: the raw games
