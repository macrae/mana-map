...
- **Changed `SYNERGY_RULES` / obsolescence thresholds / region params**: only steps 10–12 — `manamap synergy && manamap power-creep && manamap cluster-regions`. Fast (no retraining), but it invalidates **six** agent-cache routines that hash those graphs — `writer-prose`, `the-ten`, `issue-plan`, `candidate-pool`, `deck-build` and `deck-diagnosis`. Verified prose is usually still correct after a graph refresh, so re-bless (`cache-record`) rather than re-spawn; make it a stated decision.
