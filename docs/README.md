...
| **[testing.md](testing.md)** | 687 | How the suite is organised, the markers (incl. `forge`), the cache, and the lessons. **The only place that states test counts.** |
...
| [gotchas-bench.md](gotchas-bench.md) | 2346 | Agents and the invocation cache, Forge and the goldfish model, branches, the diagnostic layer, `deck-audit`, versions, the captain's log. Read before touching `src/manamap/pilot/` or `src/manamap/sim/`. |
...
| [viz.md](viz.md) | 1587 | The frontend: the six PAGES (workbench, atlas, dossier, branch, Curate, spaces), the three modes, the `window.MM` contract, the canvas renderer, seeding a walk from named cards, and what an open verified line prints. Read before any `viz/` change. |
