...
| **[known-issues.md](known-issues.md)** | 899 | **THE INVENTORY OF WHAT IS BROKEN.** The first four entries are the nine red tests and the parser bug that drops 88% of noncombat damage; the rest are gaps NO TEST FAILS ON — undecidable staleness on 19 artifacts, 50 undispatched open questions, two uncriticised engine models. (§9, the eleven-games-zero-debriefs entry, is marked RESOLVED 2026-09-11 — read the section headers, not this summary, for what is still open.) §12 lists what is already FIXED so it is not re-investigated, and a closing part tracks DEBT and OPPORTUNITIES — things that are not red but are wrong, wasteful or half-built. |
...
| **[testing.md](testing.md)** | 688 | How the suite is organised, the markers (incl. `forge`), the cache, and the lessons. **The only place that states test counts.** |
