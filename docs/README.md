...
About 12,760 lines, and 1,951 of them are history — seven files: design records, a brief
and a decklist, kept because live things cite them. It used to be roughly two thirds
history; the magazine era was deleted on 2026-08-25 and lives in git. This page says which doc is which, so you do not read a
...
| **[pilot.md](pilot.md)** | 1,813 | The bench reference: the evidence and citation contracts, every `manamap pilot` command, the per-deck artifacts, then each piece — status, `deck-info`, versions (incl. **the paper lock's three states and what a version bump means**), the captain's log and debrief, prescriptions, simulation, goldfish, scenarios and game state v2, the resolve loop, the rules and strategy DBs, facts, audit, diagnosis, engine, constellation — and a LEGACY block on the magazine renderer last. |
| **[simulation.md](simulation.md)** | 641 | Forge is the engine: the spike and its three criteria, the verdict, S1–S5 (harness, parser, the pod, the v2 bridge, the doctor reading the table), the tiers under seeding, the first runs and what they say, and the chain run once for real. |
| **[pipeline.md](pipeline.md)** | 43 | The 15 card-pipeline steps: command, inputs, outputs, runtime, when to re-run what. |
| **[data-artifacts.md](data-artifacts.md)** | 90 | Every file in `data/`: producer, size, tracked or not, who reads it — including the per-deck bench artifacts, the pod, and what is frozen legacy. Read before touching anything under `data/`. |
| **[known-issues.md](known-issues.md)** | 1,356 | **THE INVENTORY OF WHAT NO TEST FAILS ON.** `make test-fresh` is GREEN, so the page is no longer a red board: it is the list of things that are wrong and that nothing will tell you about. Undecidable staleness on 26 artifacts, 60 undispatched open questions, four sleeved decks whose engine model nobody criticised, and **five of six sleeved decks with no Forge run describing the list in their sleeves**. Four archived decks fail their own `diagnosis.json` validator. §12 lists what is already FIXED so it is not re-investigated, and a closing part tracks DEBT and OPPORTUNITIES. Audited 2026-09-21 — read the section headers, not this summary. |
...
| [gotchas-bench.md](gotchas-bench.md) | 2,390 | Agents and the invocation cache, Forge and the goldfish model, branches, the diagnostic layer, `deck-audit`, versions, the captain's log. Read before touching `src/manamap/pilot/` or `src/manamap/sim/`. |
...
| [viz.md](viz.md) | 1,630 | The frontend: the six PAGES (workbench, atlas, dossier, branch, Curate, spaces), the three modes, the `window.MM` contract, the canvas renderer, seeding a walk from named cards, and what an open verified line prints. Read before any `viz/` change. |
| [agent-cost.md](agent-cost.md) | 306 | Where LLM spend lives, per-routine token sizing (current first, legacy measurements after), and how the invocation cache decides what to re-run. |
...
and `validate_issue.py`, which rendered the nine frozen pages — **all of it deleted
2026-09-13** (`443cf6b7`), pages included. Nothing in it ever applied to the bench, and the
live renderer, the Pilot's Operating Handbook (`poh.py`), was never written against it.
