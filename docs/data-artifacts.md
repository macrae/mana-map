...
| `data/decks/<slug>/*` | build-deck / resolve-stack / write-manual / design-issue loops, `pilot goldfish`, agents | **tracked** | Curated per-deck artifacts. **Build side:** `brief.json` (authored), `candidate_pool.json` (deck-analyst), `build_plan.json` (deck-architect ⇄ deck-critic), `bracket_report.json` (◆, from `bracket-check`; `validate-build` cross-checks the plan against it), `decklist.txt` (authored *or* generated by `build-deck --write-decklist`). **Publish side:** `cards.json`, verified stacks, decisions, goldfish metrics/targets, `mana_analysis.json` (◆, from `pilot mana-analysis`), `tutor_guide.json` (At the Table's tutor subhead), `considering.json` (The Short List — FROZEN legacy since 2026-08-19; its rule lives in prescriptions), `considering_art.json` (a deterministic sidecar from `short-list-art`: Scryfall images for those ten, so the one department whose subject is OUTSIDE the deck can still show you the cards — kept out of `considering.json` because that file is the analyst's and is cache-fingerprinted per file), `diagnosis.json` (deck-doctor ⇄ deck-skeptic; axes, cuts, adds, open questions), `sim/<run-id>.json` (a Forge simulation run — ◆ SAMPLED: outcomes, per-game rows, `analysis` with CIs and its `limits`, every seat's decklist sha, the engine versions and the assumptions incl. Forge's AI caveat; `sim/logs/` beside it is gitignored and `validate-sim` re-derives the analysis from it where it exists), `prescriptions/<id>-*.json` (one question to the doctor — AUTHORED prompt + the doctor ⇄ skeptic answer in one file; accumulating, never overwritten; `validate-prescription` form-checks stale ones), `deck_recon.json` (deck-doctor MODE recon — **dated and perishable**, kept out of `strategy.md` on purpose), `strategic_frame.json`, `manual_prose.json`, `issue.json` (authored), `issue_plan.json`, `deck_map.json` (◆ positions + membership from `deck-map`, ★ city names merged by `merge-deck-map` — **tracked because `embeddings_ability.npy` is gitignored and a fresh clone must still render manuals**; positions are LOCAL to the deck, not atlas positions), `engine.json` (the eight-stage engine model from `deck-engineer` ⇄ `engine-critic`; a `critic.verdict` of `fail` is SAVED and reported as failing, never cache-recorded), `goldfish_targets.json` (the engine DECLARATION — `validate-goldfish-targets` checks it, and a fleet survey found it wrong on six of eight decks) and `goldfish_metrics.json` (◆, seeded), plus `pending.json` (hand-authored queue of decided-but-unapplied changes; closure derived from the deck, never a flag) · **the captain's log** — `log.jsonl` (AUTHORED, append-only, one game per line via `deck-notes add`, each stamped with the `decklist_sha256` of `decklist.txt` as it stood; no agent or command rewrites it) and `log_annotations.json` (DERIVED — the `debrief` agent's structured reading keyed by entry id, merged by `merge-debrief`, held to the log by `validate-debrief`: it may name nothing the note and the 99 do not) · `deck_versions.json` (AUTHORED tags on versions `deck-version` derives from git — `{tags: {name: {version, sha, decklist_sha256, at, note}}}`; the version LIST is never tracked because a commit's sha is not known inside the commit) · `HISTORY.md` (append-only decklist ledger), optional `pilot_feedback.md` (authored, a cached agent input), an optional `README.md`, and for a pool-constrained build the pool listing itself (`pool-green-bulk.txt` on radagast — the input `pool-facts` was run over, kept so the build is reproducible). **Retired:** `sideboard_analysis.json` and `upgrade_watch.json`, both folded into `considering.json`; the sideboard itself is retired — `is_sideboard` no longer exists |
