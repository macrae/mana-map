...
| **Incremental regeneration** | Card-scoped cache (2026-07-28): per-card digest map + conservative `card_refs` per routine; `STALE_OK`/`cache-rebless` clear unreferenced deck changes with zero spawns; `manamap pilot impact` reports reference/figure/target/zone staleness deterministically; per-key fingerprints (`PROSE_KEY_INPUTS`) + charter "Partial revision mode" scope spawns to stale keys/departments |
...
| `edgar-vampires` | The pilot's physical deck superseded the old builder experiment; **published** as Vol. 006 and upgraded to **v2 in place** ("THE KILL MATRIX") — verify-first-then-apply: four scenarios passed at iter 1, then seven bench promotions landed with stamps on them. Floor 4 with a fully verified **2×2 loop matrix** (Vito/Bond × Exquisite/Conqueror), **8 verified stacks** incl. one partial refutation (Throne is a 14-per-opponent drain, not a table-kill), 2 decision spreads. Ingestion caught an illegal Crosis's Catacombs (U identity) → Nomad Outpost |
