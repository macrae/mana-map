...
run once in that time because the `Test` step short-circuited it. 242 failures
to 0. The weekly `corpus-gates` job is RED ON PURPOSE until Reality Fracture
lands on 2026-10-02 (`docs/known-issues.md` §9c) — the corpus is 266 cards ahead
of the tracked artifacts, which is the index-alignment invariant working.

**All six sleeved decks now have a criticised engine model.** Four were run
through the engineer ⇄ critic loop on 2026-09-22: gishath and sharknado pass and
are recorded, ur-dragon and heliod still fail on much smaller ground (30 → 18,
23 → 19) with every round-1 finding answered. §2 has the table.

**The bench is ready to play.** Six decks sleeved and current, zero stale
artifacts, every deck's dossier recomposed. The next thing that moves this
project is games at a table, logged — `deck-notes <slug> add … --cause <code>`,
then `/debrief`.

What that week of auditing actually found, in one line each:

- A test had been writing into `data/decks/` on every run, invisible because
  with a corpus present the regenerated bytes matched (`docs/testing.md`).
- 80 tests read the corpus without declaring it, so CI CRASHED where it should
  have skipped — 159 of the 242.
- `serve.py` ran `int()` over `SystemExit.code`, which is a STRING whenever the
  pilot layer refuses, turning every refusal through `/api/cli` into a traceback.
- "39 of 53 eliminations came from the air" was never a flying attribution — it
  spliced two different fields, reached seven artifacts, and the same recipe was
  applied a second time while correcting the first (§9e).
- 60 stale claims across the live docs, most of them from the magazine delete.
...
and `experiment` is the flagship: two versions, one table, one artifact, the delta with an
interval on the DIFFERENCE of every figure — never a comparison of two marginal
intervals, which is the overlap fallacy and was deleted from the code. Around it sit a deterministic builder, a rules-citation loop for lines
...
supported. The magazine that used to be the product was DELETED on 2026-09-13; the Pilot's
Operating Handbook (`build-poh`) has rendered `manuals/p/<slug>.html` since
...
### Legacy — DELETED 2026-09-13

The magazine renderer is gone: `build_manual`, `issue_spec`, `design`,
`validate_issue`, `build_page` and seven more modules, with their seven
subcommands, nine pages and nine test files — 14,064 lines, commit `443cf6b7`.
STYLEv3 went on 2026-08-25. The Pilot's Operating Handbook (`poh.py`) had
already replaced it on 2026-09-02.

The artifacts it read (`issue_plan.json`, the panel keys,
`card_roles`/`mana_base`/`upgrades` in `manual_prose.json`, `considering.json`
and its art sidecar) are frozen where they sit on already-published decks; no
agent regenerates them and nothing gates them. What the renderer measured is in
`docs/gotchas-magazine-legacy.md`; the code is in git.
